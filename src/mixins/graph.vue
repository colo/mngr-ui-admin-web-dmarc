...
export default {
  ...
  methods: {
    ...
    update_chart_dataset: function (name, data, inmediate) {
      debug('update_chart_dataset', this.id, name, data, inmediate, this.$options['charts'][this.id].tabular.data)
      ...
      // && inmediate !== true
        ...
        debug('update_chart_dataset2', this.id, this.$options['charts'][this.id].tabular.data)
      ...
      debug('update_chart_dataset inmediate %s %o %o', this.id, inmediate, this.$options['charts'][this.id].visible, this.always_update)
      ...
      if (
        ...
        **/
        ...
        // remove timestamp
        Array.each(this.$options['charts'][this.id].tabular.data, function (row) {
        ...
        //   update_data = this.config.watch.transform(update_data, this, this.config)
        // }
        // else {
        ...
        debug('update_chart_dataset UPDATE DATA', this.id, update_data)
